...
import { Search, Menu, Video, Plus, Edit, Trash2, Play, X } from "lucide-react"
...
function YouTubeClone() {
  ...
  const addKeyword = () => {
    setNewCategory(prev => ({
      ...prev,
      keywords: [...prev.keywords, { keyword: "", weight: 25 }]
    }))
  }

  const removeKeyword = (index: number) => {
    if (newCategory.keywords.length > 1) {
      setNewCategory(prev => ({
        ...prev,
  ...
  const updateKeyword = (index: number, field: 'keyword' | 'weight', value: string | number) => {
    setNewCategory(prev => ({
      ...prev,
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    // Validate keywords
    const validKeywords = newCategory.keywords.filter(kw => kw.keyword.trim() !== "")
    ...
    // Validate weights
    const totalWeight = validKeywords.reduce((sum, kw) => sum + kw.weight, 0)
    if (totalWeight <= 0) {
    ...
    try {
      ...
      setNewCategory({ title: "", keywords: [{ keyword: "", weight: 25 }], color: "#ff0000" })
  ...
  const editCategory = (category: Category) => {
    ...
    setNewCategory({
      ...
      keywords: category.keywords.length > 0 ? category.keywords : [{ keyword: "", weight: 25 }],
  ...
  const handleVideoClick = (videoId: string) => {
    // If we have an active category, pass it to the video detail page
    if (activeCategory) {
      navigate(`/video/${videoId}?category=${activeCategory}`)
    } else {
      // If no active category (e.g., from search), navigate without category
                  ...
                  <form onSubmit={handleCategorySubmit} className="space-y-4">
                    ...
                    <div>
                          ...
                          <div key={index} className="flex items-center space-x-2">
                            ...
                            <div className="w-20">
                              <Input
                                ...
                                onChange={(e) => updateKeyword(index, 'weight', parseInt(e.target.value) || 0)}
                                className="text-sm"
                                min="0"
                                max="100"
                              />
                            </div>
                            {newCategory.keywords.length > 1 && (
                              <Button
                                type="button"
                                size="sm"
                                variant="ghost"
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Total: {newCategory.keywords.reduce((sum, kw) => sum + kw.weight, 0)}%
                      </p>
                    </div>
              ...
              {categories.map((category) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500">
                    {category.keywords.map((k) => `${k.keyword} (${k.weight}%)`).join(", ")}
                  </p>
                  <Button
                    size="sm"
                    variant="outline"
